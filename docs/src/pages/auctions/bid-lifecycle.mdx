...
The operator can optionally disclose the winning group's committed refund
recipient to an authenticated consumer after settlement. This is an offchain
application convention, not a Cairo fulfillment rule: it is appropriate only
when the bidder intentionally chose the same address for refunds and prize
control. Losing recipients are never returned by that endpoint.
...
It does **not** move funds and does not let bidders reclaim vault notes. The same operator still has to construct private refunds. Rejected, unfunded notes use the operator's [rejected-bid recovery and abort runbook](/operations/rejected-bid-recovery), which commits the confirmed refund transaction as the recovery hash; accepted-note recovery after an abort remains separate work. This is an operational recovery marker, not a cryptographic escape hatch.
