...
The pool's `EncNoteCreated` event exposes a note ID. Whisper intentionally records the accepted note ID too, because it needs a durable handle connecting the funded bid to the vault's later settlement input.
