...
flowchart LR
  ...
  bidder -->|"wallet transfer + invoke"| pool
  pool -->|"encrypted note + commitments"| whisper
...
Whisper does not deploy a second privacy pool and does not require an auction-specific prover circuit. Bidder dapps send the standard Wallet API `transfer` + `invoke` actions, so the connected wallet owns proving. The operator still uses `ComputeAndInvoke` for authenticated vault acceptance and settlement.

## One auction in 60 seconds

1. The bidder starts with funds already shielded in the STRK20 pool.
2. The dapp gives a compatible wallet an exact-value private transfer plus a Whisper invoke containing commitments.
...
5. The bidder may add another note tranche to increase the same logical bid.
6. After the deadline, the operator opens all accepted tranches, sums each group, and computes the Vickrey result.
7. One private settlement spends the escrow notes, refunds losing groups, returns the winner's change, and pays the seller.
8. Whisper verifies the accepted tranche set, every opening, the group totals, winner, and clearing price.
...
| Bidder wallet and bid nonce | Group/tranche handles and commitments |
| Refund destination and winner payload | Note ID, timing, submission count, funded-tranche count |

Settlement reveals the accepted tranche amounts, aggregate winner, and clearing price. The public chain still does not learn which wallets own the bid notes or private settlement outputs.
...
The Cairo package is application-neutral. Every auction explicitly selects offchain, ERC-20, ERC-721, or ERC-1155 fulfillment: a game can interpret `winner_commitment` as control of an arbiter, while Whisper can escrow a token lot for delivery to the committed winner. [Read the fulfillment model](/auctions/fulfillment).
