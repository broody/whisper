...
|---|---|---|
| Public observer | Commitments, note ID, timing, counts | Accepted amounts, winner commitment, clearing price, transcript roots; a consumer may separately publish the winning refund address |
...
| Bidder wallet, viewing key, notes, and bid nonce | Group/tranche handles and commitments |
| Vault note token/amount to ordinary observers | Accepted note ID |
| Refund destination | Submission/funded-tranche counts and timestamps |
...
| Losing refund, change, and seller note recipients | Winning bid, second-highest bid, clearing price |
| Vault viewing key and channel keys | Winner commitment and settlement roots |

An authenticated consumer may opt into the operator's post-settlement winner
endpoint when its prize convention uses the refund recipient as the winner.
The operator then discloses only that winning address; the consumer decides
whether to publish it. This adds no new bidder information to the operator,
which already decrypts every capsule, but it intentionally makes the winner
public if the consumer republishes the result.
...
- group and tranche handles cannot be submitted twice;
- only the committed operator identity can accept, settle, or abort;
- accepted note IDs cannot be reused;
- the complete ordered funded-tranche set is used;
- every published tranche amount opens its bid-time commitment;
- tranche amounts are summed by their committed group before pricing;
- the winner and tie-break are deterministic; and
- the clearing price is correct.

It does not hold or decrypt the notes.

The bidder and operator use different proving boundaries. A compatible wallet proves bidder `transfer` + `invoke` actions without exposing the user's viewing key to the dapp or Whisper backend. The operator's proving service receives only vault-owned acceptance and settlement work; moving bidder submission to the Wallet API does not remove the operator prover requirement.

The deployed v0.4 contract is upgradeable. The two-step owner can replace its class hash and therefore change any of these contract guarantees for future calls. Two-step transfer prevents authority from moving to an unintended address without acceptance, but it does not constrain what an accepted owner may upgrade to. Production ownership should use independently reviewed governance or a timelocked multisig, and every proposed class should be reviewed for storage compatibility and behavior before execution.
...
## Asset escrow does not remove payment custody

`WhisperAuction` can hold an ERC-20, ERC-721, or ERC-1155 lot and release it only to the recipient opening the verified winning commitment. This removes the seller's ability to refuse delivery of that deposited token.

It does not verify that the private proceeds note was actually created for the seller. A malicious operator could still settle the mathematically correct auction while redirecting or withholding private outputs, after which the winner could claim the onchain lot. Until output binding exists, both bidders and sellers remain inside the operator trust boundary. The public also sees the lot deposit, token details, and eventual claim recipient; the asset side is not private.
...
| Transaction creates multiple notes with the committed bid amount | Submission is rejected as ambiguous; unrelated change is allowed |
| Acceptance has no replay-protecting private action | Pool rejects the proof; operator must rotate a separate vault replay note |
| Operator misses the acceptance deadline | Bid stays unfunded and can be privately returned with the operator recovery command |
...
| Upgrade owner installs malicious or incompatible code | Contract guarantees or stored auction state may be changed or broken |
