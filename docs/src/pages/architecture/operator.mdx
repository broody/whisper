...
| `operator/src/recovery.ts` | Verifies and returns exact notes for rejected, unfunded bids |
| `operator/src/recover-auction.ts` | Provides the dry-run-first manual recovery command |
| `operator/src/sqlite-store.ts` | Persists capsules, submissions, auctions, recoveries, leases, cursor, and idempotency state |
| `operator/src/api.ts` | Exposes health, public vault configuration, idempotent capsule upload, authenticated coordination, and post-settlement winner disclosure endpoints |
...
## Optional winner disclosure

When a coordinator token is configured, an authenticated consumer may call
`GET /v1/auctions/<auction-id>/winner`. Before settlement it returns no address.
After settlement, the operator reads the canonical result, selects only the
winning group, decrypts its stored capsules, and revalidates the reveal,
winner, and refund commitments before returning the committed refund
recipient. This supports applications that intentionally use the same address
for refunds and prize control without publishing losing recipients or storing
a browser-side claim secret.
...
Rejected, unfunded notes can be returned with the dry-run-first [recovery command](/operations/rejected-bid-recovery). After the deadline, its confirmed transaction can be committed by the dry-run-first abort command. Production still needs alerting well before the abort deadline, automatic abort/funded-auction recovery, database backups, and a single scheduler leader per database.
